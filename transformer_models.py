...
def get_cnn_model(image_size: int) -> tf.keras.Model:
    ...
    #shape = base_model_out.shape
    #base_model_out = tf.reshape(base_model_out, shape=[-1, shape[1] * shape[2], shape[3]])
...
class TransformerDecoderBlock(tf.keras.layers.Layer):
    def __init__(self, embed_dim: int, ff_dim: int, num_heads: int, sequence_length: int, vocab_size: int, **kwargs):
        super().__init__(**kwargs)
        self.embed_dim = embed_dim
        self.ff_dim = ff_dim
        self.num_heads = num_heads
        ...
        self.embedding = PositionalEmbedding(
            embed_dim=embed_dim, sequence_length=sequence_length, vocab_size=vocab_size
        )
...
class ImageCaptioningModel(tf.keras.Model):
    def __init__(
        self, cnn_model: keras.Model, encoder: TransformerEncoderBlock,
            decoder: TransformerDecoderBlock, num_captions_per_image = 5,
    ):
        super().__init__()
